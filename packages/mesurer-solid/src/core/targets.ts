...
export const pickMultiTargets = (selectionRect: Rect, items: Array<{ element: HTMLElement; rect: Rect }>) => {
  const candidates: MultiCandidate[] = items.map(({ element, rect }) => {
    const overlapArea = intersectionArea(selectionRect, rect); const elementArea = Math.max(1, rectArea(rect));
    return { element, rect, overlapArea, elementCoverage: overlapArea / elementArea };
  }).filter(({ overlapArea, elementCoverage }) => overlapArea > 0 && elementCoverage >= MIN_MULTI_ELEMENT_COVERAGE);
  const pruned = candidates.filter((candidate, index) => !candidates.some((other, otherIndex) => {
    if (index === otherIndex) return false;
    const candidateArea = rectArea(candidate.rect); const otherArea = rectArea(other.rect);
...
export const pickSingleTarget = (selectionRect: Rect, point: Point, items: Array<{ element: HTMLElement; rect: Rect }>) => {
  ...
  const scored = items.map(({ element, rect }) => {
    const overlap = intersectionArea(selectionRect, rect); const elementArea = Math.max(1, rectArea(rect)); const coverage = overlap / elementArea;
    const areaRatio = elementArea / selectionArea; const areaSimilarity = 1 - Math.min(1, Math.abs(Math.log(areaRatio)) / 2);
    ...
    return { element, rect, coverage, score: coverage * 0.55 + areaSimilarity * 0.35 + (pointerInside ? 0.2 : 0) - largeContainerPenalty };
  }).filter(({ rect, coverage }) => rect.width >= MIN_SINGLE_TARGET_SIZE && rect.height >= MIN_SINGLE_TARGET_SIZE && coverage >= MIN_SINGLE_ELEMENT_COVERAGE);
  if (scored.length === 0) return null; scored.sort((a, b) => b.score - a.score); return scored[0].element;
};
export const pickPointTarget = (point: Point, items: Array<{ element: HTMLElement; rect: Rect }>) => {
  const scored = items.filter(({ rect }) => rect.width >= MIN_SINGLE_TARGET_SIZE && rect.height >= MIN_SINGLE_TARGET_SIZE).map(({ element, rect }) => {
    const area = Math.max(1, rectArea(rect)); const inside = point.x >= rect.left && point.x <= rect.left + rect.width && point.y >= rect.top && point.y <= rect.top + rect.height;
    const distance = getDistanceToRect(point, rect); const proximity = 1 / (1 + distance); const areaWeight = 1 / (1 + Math.log(area));
    return { element, score: (inside ? 2 : 0) + proximity * 0.9 + areaWeight * 0.35 };
  });
  if (scored.length === 0) return null; scored.sort((a, b) => b.score - a.score); return scored[0].element;
};
