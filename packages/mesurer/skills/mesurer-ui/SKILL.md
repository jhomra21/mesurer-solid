...
Use the transport owned by the registered Codex session. For Codex Desktop, the trusted SessionStart connector carries `CODEX_APP_TOOLS_PIPE_PATH` and `CODEX_HOME` to the local bridge. Persist the feedback in Mesurer's local Desktop queue, wait while the app-owned `read_thread` reports the target `active`, and invoke `send_message_to_thread` only after observing `idle` or `notLoaded`. Do not start the standalone app-server daemon for Desktop. Desktop queue state survives bridge restart in `$CODEX_HOME/mesurer/codex-deliveries.json`.

For CLI/TUI shared-daemon sessions, keep Codex's native `codex queue` transport and queued-submission id. If migrating an older native queue item into a Desktop-owned delivery, validate the exact queue id, persist the full message locally first, then delete only that exact native item before app-native dispatch.

Desktop currently exposes no atomic queue-only cross-thread app-tool call, so the idle check and send can race. Do not claim a stronger guarantee. Avoid knowingly steering an active turn, never blindly retry an uncertain Desktop send, and keep the delivery disabled as blocked while confirmation is uncertain.
