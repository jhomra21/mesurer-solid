...
Delivery is queue-only today. `codex queue` preserves the active turn and appends a follow-up; after durable acceptance, the companion checks the same shared app-server daemon and resumes only a `notLoaded` destination so a cold queue can dispatch. Already-loaded threads stay under Codex's scheduler. Codex's separate **Steer** action targets an in-flight turn. Do not describe Mesurer delivery as steering or assume that the short-lived app-server used for thread discovery owns the target's active turn.
