...
Delivery is queue-only today. `codex queue` preserves the active turn and appends a follow-up. After durable acceptance, the companion checks the shared local app-server daemon. If its control socket is missing, Mesurer runs Codex's idempotent `app-server daemon start` command and retries; it does not bootstrap or install the daemon. The bridge resumes only a `notLoaded` destination, while already-loaded threads stay under Codex's scheduler. Codex's separate **Steer** action targets an in-flight turn. Do not describe Mesurer delivery as steering or assume that the short-lived app-server used for thread discovery owns the target's active turn.
