...
Delivery is queue-only today. `codex queue` preserves the active turn and appends a follow-up. After durable acceptance, the companion checks the shared local app-server daemon. If its control socket is missing, Mesurer runs Codex's idempotent `app-server daemon start` command and retries. Codex may prepare its managed daemon package from the current complete CLI during that official start. Mesurer does not call `daemon bootstrap` or enable remote control. The bridge resumes only a `notLoaded` destination, while already-loaded threads stay under Codex's scheduler. Queued deliveries carry Codex's durable queued-submission id in per-tab state so a restarted bridge can validate and reconstruct the delivery without calling `codex queue` again. Legacy state without the id is recoverable only when the target thread has exactly one queued submission. Codex's separate **Steer** action targets an in-flight turn. Do not describe Mesurer delivery as steering or assume that the short-lived app-server used for thread discovery owns the target's active turn.
