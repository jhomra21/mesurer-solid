...
For Vite, put this in the existing browser entry such as `src/main.tsx`, `src/main.ts`, or `src/index.tsx`. In Electron, use the renderer entry. Keep privileged Electron work in main or preload. The Screenshot plugin can consume PNG bytes from that bridge through `createElectronScreenshotCaptureProvider()`. In SSR applications, mount from a client-only module or lifecycle.
