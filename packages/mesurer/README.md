...
This uses Codex's own queued-user-message command. **Queue to Codex** does not interrupt an active turn. After durable queue acceptance, the bridge checks the shared local app-server daemon. If the control socket is missing, it runs Codex's idempotent `app-server daemon start` command and retries. Codex may prepare its managed daemon package from the current complete CLI during that official start. Mesurer does not call `daemon bootstrap` or enable remote control. A cold `notLoaded` destination is then resumed, while already-loaded threads keep Codex's normal queue scheduling. Queued delivery state also survives a bridge restart: the page persists Codex's durable queued-submission id and the restarted bridge validates that item against Codex before reattaching. Older page state without the id is restored only when exactly one queued submission exists for the target thread. Codex's separate **Steer** action targets an in-flight turn and Mesurer does not invoke it today. Programmatic `send()` resolves with `delivery: "queued"` and, on current Codex builds, includes the durable queued-submission id plus bridge dispatch metadata.
