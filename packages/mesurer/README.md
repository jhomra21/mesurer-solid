...
**Queue to Codex** follows the execution owner. Codex Desktop sessions register their app-tools pipe through the trusted SessionStart connector. Mesurer persists Desktop feedback locally, waits while `read_thread` reports an active target, and submits through the app-owned `codex_app` transport only after observing idle/not-loaded state. It does not start or require the standalone app-server daemon for Desktop. CLI/TUI shared-daemon sessions keep Codex's native `codex queue` path and queued-submission id.

Desktop's current app-tools surface does not provide an atomic queue-only cross-thread call, so the idle check and send are best-effort. Mesurer never knowingly steers an active turn, verifies a possibly lost send acknowledgement before deciding it failed, and never blindly retries an uncertain send. Desktop feedback survives a bridge restart in `$CODEX_HOME/mesurer/codex-deliveries.json`; an older native queue item can be migrated by exact id without re-enqueueing. Programmatic `send()` resolves with `delivery: "queued"` plus bridge dispatch metadata.
