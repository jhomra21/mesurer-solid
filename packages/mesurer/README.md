...
This uses Codex's own queued-user-message command. **Queue to Codex** does not interrupt an active turn. After durable queue acceptance, the bridge checks the shared local app-server daemon. If the control socket is missing, it runs Codex's idempotent `app-server daemon start` command and retries; it never bootstraps or installs the daemon automatically. A cold `notLoaded` destination is then resumed, while already-loaded threads keep Codex's normal queue scheduling. Codex's separate **Steer** action targets an in-flight turn and Mesurer does not invoke it today. Programmatic `send()` resolves with `delivery: "queued"` and, on current Codex builds, includes the durable queued-submission id plus bridge dispatch metadata.
