...
const isInspectMeasurement = (measurement: Measurement | InspectMeasurement): measurement is InspectMeasurement =>
  "paddingRect" in measurement;
const rectTransition = (measurement: Measurement | InspectMeasurement) => isInspectMeasurement(measurement)
  ? "none"
...
const labelTransition = (measurement: Measurement | InspectMeasurement) => isInspectMeasurement(measurement)
  ? "none"
...
export function MeasurementBox(props: MeasurementBoxProps) {
  ...
  return (
    <Show when={props.measurement}>
      {(measurement) => <div class="msr:pointer-events-none" data-mesurer-measurement="true" data-mesurer-selected-measurement={isInspectMeasurement(measurement()) ? "true" : undefined} data-mesurer-selection-group={isSelectionGroup() ? "true" : undefined}>
        <Show when={!isSelectionGroup()}>
          <div class="msr:absolute" style={{
            ...
            transition: rectTransition(measurement()),
          }}>
        ...
        <div class="msr:pointer-events-none msr:absolute msr:rounded msr:px-1 msr:py-0.5 msr:text-[10px] msr:text-ink-50 msr:tabular-nums msr:select-none msr:-translate-x-1/2 msr:bg-ink-900/90" style={{
          ...
          transition: labelTransition(measurement()),
        }}>
