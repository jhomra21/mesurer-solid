...
export function createTextInspector(options: TextInspectorOptions = {}, legacy = false): TextInspectorAPI {
  ...
  const record = () => { history.push(snapshot());

 if (history.length > 100) history.shift(); future.length = 0; };
  ...
  const undo = () => { const previous = history.pop();

 if (!previous) return false; future.push(snapshot()); restore(previous);

 return true; };

  const redo = () => { const next = future.pop();

 if (!next) return false; history.push(snapshot()); restore(next);
