...
export const getSnapGuidePosition = (params: { orientation: Guide["orientation"]; point: Point; snapGuidesEnabled: boolean; overlayNode: HTMLDivElement | null; guides: Guide[]; draggingGuideId: string | null; document?: Document }) => {
 ...
 if (rect.width <= 2 || rect.height <= 2) continue;
    ...
    for (const candidate of candidates) { const distance = orientation === "vertical" ? Math.abs(candidate - point.x) : Math.abs(candidate - point.y);

 if (distance <= GUIDE_SNAP_DISTANCE && distance < bestDistance) { bestValue = candidate; bestDistance = distance; } }
  }

  for (const guide of guides) { if (guide.id === params.draggingGuideId || guide.orientation !== orientation) continue; const distance = Math.abs(guide.position - (orientation === "vertical" ? point.x : point.y));

 if (distance <= GUIDE_SNAP_DISTANCE && distance < bestDistance) { bestValue = guide.position; bestDistance = distance; } }
...
export const getNearestElementToGuide = (params: { guide: Guide; overlayNode: HTMLDivElement | null; document?: Document }) => {
 ...
 if (rect.width < MIN_SINGLE_TARGET_SIZE || rect.height < MIN_SINGLE_TARGET_SIZE) continue; const distance = params.guide.orientation === "vertical" ? position < rect.left ? rect.left - position : position > rect.left + rect.width ? position - (rect.left + rect.width) : 0 : position < rect.top ? rect.top - position : position > rect.top + rect.height ? position - (rect.top + rect.height) : 0; const area = rect.width * rect.height;

 if (distance < bestDistance || (distance === bestDistance && area < bestArea)) { bestDistance = distance; bestArea = area; bestElement = element; } }
