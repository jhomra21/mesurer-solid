...
export const getHoveredGuide = (point: Point | null, guides: Guide[]) => { if (!point || !guides.length) return null; let closest: Guide | null = null; let closestDistance = GUIDE_SNAP_DISTANCE + 1;

 for (const guide of guides) { const distance = getGuideDistance(guide, point);

 if (distance <= GUIDE_SNAP_DISTANCE && distance < closestDistance) { closest = guide; closestDistance = distance; } }
...
export const getOptionPairOverlay = (params: { document?: Document; window?: Window; altPressed: boolean; primarySelectedMeasurement: InspectMeasurement | null; selectedGuide: Guide | null; hoverGuide: Guide | null; hoverElement: Element | null; selectedElementRef: Element | null }) => { const ownerWindow = params.window ?? window;
 ...
 if (selectedTarget.element && hoverTarget.element && (selectedTarget.element === hoverTarget.element || selectedTarget.element.contains(hoverTarget.element) || hoverTarget.element.contains(selectedTarget.element))) return null;

 if (selectedTarget.guideId && hoverTarget.guideId && selectedTarget.guideId === hoverTarget.guideId) return null;
