...
const normalizeSelectionSpacingStyle = (value: unknown): SelectionSpacingStyle | undefined => { if (!value || typeof value !== "object") return undefined; const input = value as Record<string, unknown>; const line = normalizeLineStyle(value, DEFAULT_SELECTION_SPACING_STYLE); if (!line) return undefined; return { ...line, enabled: typeof input.enabled === "boolean" ? input.enabled : DEFAULT_SELECTION_SPACING_STYLE.enabled, color: typeof input.color === "string" ? input.color : DEFAULT_SELECTION_SPACING_STYLE.color, diagonals: typeof input.diagonals === "boolean" ? input.diagonals : DEFAULT_SELECTION_SPACING_STYLE.diagonals }; };
...
export const normalizeStoredWorkspace = (value: unknown): MesurerStoredWorkspace | null => {
  ...
  if (typeof input.enabled !== "boolean" || !validMode || typeof input.rulersVisible !== "boolean" || (input.guideOrientation !== "vertical" && input.guideOrientation !== "horizontal") || !Array.isArray(input.guides) || !Array.isArray(input.selectedGuideIds) || !Array.isArray(input.measurements) || input.activeMeasurement === undefined || !Array.isArray(input.heldDistances)) return null;
