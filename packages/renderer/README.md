...
The public package is `@jhomra21/mesurer`. It bundles this renderer and its Solid 2 runtime into an isolated browser island so the host application can use Solid 1, Solid 2, React, Vue, Svelte, vanilla DOM, or an Electron renderer.
...
For repository development, use the root scripts. Renderer-specific checks can be run with the `@jhomra21/mesurer-renderer` workspace filter.
