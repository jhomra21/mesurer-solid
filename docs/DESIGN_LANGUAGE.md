...
Use the existing surface classes and tokens before adding feature-specific chrome.

- Floating white controls, menus, inspectors, and review cards use an 8px radius and the shared `--msr-shadow-floating` shadow.
- Small controls use the 5px `--msr-radius-control` radius unless their geometry has a stronger interaction reason.
- The toolbar uses the same shadow language but keeps its own motion and clipping structure.
...
Do not add a new one-off shadow or floating-card radius without a product reason.
