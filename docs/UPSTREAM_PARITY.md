...
| Centralized floating-surface shadow/radius polish | **Adopted after 0.1.8** | Mesurer Solid now carries the current upstream floating shadow and control-radius tokens as an explicit design-language contract. Context and Typography floating cards were aligned to the shared 8px surface geometry while preserving their Solid-specific ownership and placement behavior. |
