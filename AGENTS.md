...
- Codex delivery lifecycle must stay single-flight in the browser, keep hook registration local-process-only, verify bridge source identity before process reuse, preserve page-local routing and in-flight delivery state across same-tab reloads, refuse ambiguous multi-thread defaults, and remove only exact sent annotation ids after a matched completed turn; interruption or uncertain lifecycle state must preserve review evidence;
