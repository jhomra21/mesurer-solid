...
- Route **Queue to Codex** through the session's real owner: Codex Desktop registers its app-tools pipe, persists queued Mesurer feedback locally across bridge restarts, waits for the target to become idle before app-native delivery, verifies lost acknowledgements before retry decisions, and migrates older exact native queue items without duplicates; CLI/TUI shared-daemon sessions retain Codex's native durable queue path.
