...
- Make Screenshot choose its capture path internally. Electron/native hosts can expose `window.__MESURER_HOST__.captureScreenshot`, the Chromium extension keeps its own adapter, and `getDisplayMedia()` remains the browser fallback. Renderer usage stays `screenshot()`; host capture selection stays private, while the previous capture-provider and low-level Screenshot helpers remain only as deprecated compatibility surface. Add a real Electron 43 package-smoke contract with context isolation and sandboxing enabled, node integration disabled, and packaged `file://` coverage.
