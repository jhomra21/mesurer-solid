...
- Make Screenshot choose its capture path internally. Electron/native hosts can expose `window.__MESURER_HOST__.captureScreenshot`, the Chromium extension keeps its own adapter, and `getDisplayMedia()` remains the browser fallback. Renderer usage stays `screenshot()`; the draft-only Electron provider factory and public `capture` option were removed. Add `captureScreenshotPng` as the host-neutral low-level helper while retaining `captureVisibleTabPng` as a compatibility alias. Add a real Electron 43 package-smoke contract with context isolation and sandboxing enabled, node integration disabled, and packaged `file://` coverage.
