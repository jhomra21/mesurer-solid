...
- Keep Codex queue delivery live for cold destinations: retain Codex's durable queued-submission id, inspect the shared local app-server daemon after `codex queue`, start that daemon through Codex's idempotent lifecycle command when its control socket is absent, resume only `notLoaded` threads, keep wake failures non-fatal, and reattach queued deliveries after a bridge restart by validating Codex's persisted queue instead of submitting accepted feedback twice.
