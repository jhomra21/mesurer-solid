...
**Queue to Codex** is literal: it adds a follow-up without interrupting the active Codex turn. After Codex durably accepts the queue item, the companion checks the shared local app-server daemon. If its control socket is missing, Mesurer runs Codex's idempotent `app-server daemon start` command and retries. Codex may prepare its managed daemon package from the current complete CLI during that official start. Mesurer does not call `daemon bootstrap` or enable remote control. A cold `notLoaded` destination is then resumed so the persisted queue can dispatch; already-loaded threads stay under Codex's scheduler. If the local bridge restarts while feedback is still queued, the page reattaches to Codex's durable queued-submission id instead of submitting the message again. Legacy page state without that id is restored only when the target thread has exactly one queued submission. Codex may show its own **Steer** action on that queued message; Mesurer does not invoke `turn/steer` today. Programmatic `send()` reports `delivery: "queued"` and exposes Codex's queued-submission id plus dispatch metadata when available.
