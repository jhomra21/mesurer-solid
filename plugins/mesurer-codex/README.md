...
Its `SessionStart` hook runs for `startup`, `resume`, and `clear`. The hook reads Codex's `session_id` from stdin, starts or reuses the loopback bridge at `127.0.0.1:47365`, and registers that session as the active **Send to Codex** destination. Successful startup is silent, so the hook does not add developer context to the Codex session.
...
After trust, new, resumed, and cleared Codex sessions start or reuse the bridge automatically. The browser-side Mesurer Codex toggle still controls only the browser plugin. Turning it off removes **Send to Codex** from that page but does not kill the shared local bridge.
...
Then enable Codex in Mesurer Settings and use **Send to Codex**. No separate `mesurer-codex` terminal should be needed.
