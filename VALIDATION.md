...
Arrange interaction changes must use the real Select path. With a selected Arrange target, the first Escape clears the selection without disabling Arrange or Select. Select hover chrome stays hidden while that selection exists, then resumes after Escape clears it. A second Escape with nothing selected must turn off Arrange and Select. Plain Select follows the same two-stage selection lifecycle. Nested parent and child targets must not look simultaneously selected.

Arrange multi-selection acceptance must use physical Shift input. After two separated targets create a group box over a third page element, Shift-clicking that covered element must extend the selection. A nested parent and child must move by the same rendered delta as the Arrange group box. Stored intent values alone are not enough evidence because ancestor transforms can change descendant geometry.

Arrange movement must also be tested against host CSS transitions. Use selected targets with different `transition: all` durations, sample their rendered rectangles while the pointer is still down, then sample again after release. Every target must match the group-box delta during the drag and must remain stationary after pointer release. Removing the Arrange preview must restore the host transition property and duration.

Arrange reset acceptance must exercise real pointer input and rendered geometry. A moved selected element must expose its 24px Reset position control, resetting one element must restore its original geometry and remove that element's saved position, and Reset all positions must restore accumulated current-page moves. The browser contract writes its geometry evidence and reset-button screenshot to the retained Arrange artifact.
