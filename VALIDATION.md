...
Arrange interaction changes must use the real Select path. With a selected Arrange target, Escape clears the selection without disabling Arrange or Select. Select hover chrome stays hidden while that selection exists, then resumes after Escape clears it. Nested parent and child targets must not look simultaneously selected.
