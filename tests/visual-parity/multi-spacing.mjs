...
try {
  ...
  const sparseEvidence = await page.evaluate(async () => {
    ...
    for (const root of spacingRoots) {
      const label = root.textContent?.trim() ?? "";

      if (label) spacingLabels.push(label);
    }
  ...
  const evidence = await page.evaluate(async () => {
    ...
    for (const labelElement of document.querySelectorAll(
      '[data-mesurer-distance-kind="selection-spacing"] [data-mesurer-distance-label="true"]',
    )) {
    ...
    spacingLabels.sort((a, b) => Number(a) - Number(b));
