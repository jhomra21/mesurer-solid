...
Delivery uses two owner-specific transports. When `SessionStart` exposes `CODEX_APP_TOOLS_PIPE_PATH`, the destination belongs to Codex Desktop. The connector records that pipe and `CODEX_HOME`; the bridge discovers Desktop's installed `codex_app` MCP launcher, persists the full queued feedback locally under `$CODEX_HOME/mesurer/codex-deliveries.json`, waits while `read_thread` reports `active`, and calls `send_message_to_thread` only after observing `idle` or `notLoaded`. A bridge restart reloads the same local queue. If an older Mesurer build left the same feedback in Codex's native queue, migration validates the exact queued-submission id, persists the message locally, and deletes only that exact native item before Desktop dispatch. A failed Desktop acknowledgement is verified with an immediate thread read before the bridge marks the send uncertain; uncertain sends are never blindly retried.

When no Desktop app-tools pipe is registered, Mesurer keeps the CLI/TUI shared-daemon path: `codex queue` owns durable persistence, the bridge retains Codex's queued-submission id, and a `notLoaded` destination may be resumed through the shared daemon so Codex drains that queue. Mesurer does not bootstrap or require that standalone daemon for Desktop.

Queue and Steer remain separate concepts. Desktop's current app-tools surface does not expose an atomic queue-only cross-thread call, so Mesurer avoids knowingly steering by waiting for an idle/not-loaded status before app-native send. That check and send are not atomic; the architecture does not claim a stronger guarantee than the owner exposes.
