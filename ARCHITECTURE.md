...
Mesurer does not create a Codex thread or call `turn/start`. New threads are created or opened in Codex, whose trusted `SessionStart` path can then register them. The delivery path continues to use `codex queue` and does not take a second writer lock. After durable queue acceptance, the companion queries the shared local daemon through Codex's stdio-to-UDS relay. If the control socket is absent, it runs the idempotent `codex app-server daemon start` command and retries. Codex may prepare its managed daemon package from the current complete CLI during that official start. Mesurer does not call `daemon bootstrap` or enable remote control. The bridge calls `thread/resume` only for a `notLoaded` destination, while loaded threads remain under Codex's queue scheduler. Browser state keeps the durable queued-submission id alongside the delivery id. After a bridge restart, recovery queries `thread/queue/list`, validates the exact queued item, reconstructs the delivery hash from Codex's stored text, and resumes tracking without re-enqueueing. Legacy state with no queue id is accepted only when the thread has exactly one queued item. Queue and Steer are separate Codex operations: current `codex queue` routes through the app-server `thread/queue/add` path, while in-flight steering is `turn/steer`. Mesurer does not currently attach to the active-turn owner, so it must not present queue delivery as steering.
